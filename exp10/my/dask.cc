   ...
   InitSpace("SCAN");
    ...
    for(int i=0; i<RequestNumber; i++){
       ...
       for(int j=0;j<RequestNumber;j++){
          ...
          if(Cylinder[j]==-1)//已经扫描过该磁道了

           continue;

           else if((Direction==0&&Cylinder[j]<Current&&(Current-Cylinder[j])<Distance)

           ||(Direction==1&&Cylinder[j]>Current&&(Cylinder[j]-Current)<Distance)){//找到一个同方向最近的一个磁道
       ...
       if(Direction==0){
        ...
        }else{//如果同方向上没有请求的磁道了，就跳到0号磁道并转向
   ...
   InitSpace("Look");
    ...
    for(int i=0; i<RequestNumber; i++){
       ...
       if(Direction==0){
        ...
        //与scan算法的区别在于如果当前方向前方没有请求的磁道了，就掉头，不再走到尽头了
